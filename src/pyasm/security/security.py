...
class Site(object):
    ...
    def allow_guest(cls, url=None):
        return None
    allow_guest = classmethod(allow_guest)

    def get_guest_hashes(cls):
        return []
    get_guest_hashes = classmethod(get_guest_hashes)


    def get_user_project(cls):
        return Config.get_value("install", "default_project")
    get_user_project = classmethod(get_user_project)
    ...
    def build_ticket(cls, ticket):
        '''Provide the opportunity to alter the ticket customized for the site'''
        return ticket
